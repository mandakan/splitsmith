...
- The confidence formula (in `candidate_confidence`) blends tonal purity, duration plausibility, and saturating silence preference, tilted by the margin to the runner-up. Empirically validated against `tests/fixtures/beep_calibration/`: confidence >= 0.7 is right ~95 % of the time. The HTTP server / MCP / SPA use this against `automation.beep_low_confidence_threshold` (default 0.95; raised from 0.6 in #296 after field use auto-trusted too many wrong beeps) to decide whether the auto-trust chain fires (#219).
...
- `project_loader.py`: `load_shooter` walks `MatchProject.stages`, derives the per-stage trim path via the same `_slugify` rule the per-stage exporter uses, and computes `beep_offset_in_clip = min(trim_pre_buffer_seconds, primary.beep_time)`. Stages without a primary, beep, trim, or marked `skipped` are omitted. A stage dropped for a *missing trim* is also recorded in `missing_trims`, which the CLI prints as a warning -- a black filler tile is otherwise indistinguishable from "this shooter didn't shoot the stage".
...
**`camera_select.py`** — Per-shooter camera selection for trim and compare exports. A selector is a `camera_mount` ("chest") or a role ("primary" / "secondary"); resolution is mount-first so a user who tags mounts gets the obvious behaviour, with roles as the fallback for untagged projects.
- `resolve_camera(videos, camera)`: the video this selector names on one stage, or `None` when it isn't there (normal -- a cam was forgotten or its battery died -- and callers substitute the primary). Raises `CameraResolutionError` when the selector is *ambiguous* on the stage: two secondaries under `"secondary"`, or two videos wearing the same mount tag. Ingest order must never decide which camera the user gets.
- `validate_camera(stages_videos, camera)`: whole-project check, run once. Resolvable on at least one stage is enough; a value matching nothing anywhere is a typo or a stale config and fails loudly rather than silently exporting every tile from the primary.
- `parse_camera_overrides(pairs)`: parses the CLI's repeatable `SHOOTER=VALUE`.

**`match_trims.py`** — Trim-only export across every shooter in a match; produces the lossless per-stage trims a compare grid needs, from a beep and a stage time alone. No shot detection: the grid's emitter never reads shot data, and `exports.export_stage` treats a missing audit as zero shots.
- `plan_trims(...)`: pure -- reads each shooter's `project.json` and classifies, touching no media (it asks `MatchProject.source_present`, which never mirrors a hosted object). `--dry-run` therefore shows exactly what a real run would do.
...
Because the diff is against `Match.stages`, the editor reads `Match.stages` too, via `GET /api/match/stages`. A shooter's `project.stages` is a different document and is not a substitute: linking a scoreboard rewrites `match.stages` with the scoreboard's names and `stage_rounds` while leaving each shooter project on its placeholder names, and no later sync closes that gap. Submitting a shooter's copy therefore reads as a rename of every stage, reverting names and stripping `stage_rounds` -- which the ensemble consumes as the `expected` prior.

The fan-out is not atomic: a lost optimistic lock on shooter N's save returns 409 with shooters 1..N-1 already written and the match document untouched. Applying an edit is therefore idempotent -- both append paths skip a `stage_number` the target list already holds -- so an identical retry cannot give a shooter two entries sharing one number, and one artifact key.

Derived caches are resolved per shooter, at `<match_root>/shooters/<slug>/{audio,trimmed}/`. There is no single root for the purge to walk.
