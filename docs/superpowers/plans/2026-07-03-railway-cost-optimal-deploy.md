...
- **Steady enqueues can starve drains through the mid-boot blind spot:** with the 30s cooldown shorter than boot-to-heartbeat (~60-90s), enqueues arriving every 30-90s can keep replacing booting workers so nothing drains until the user pauses. Self-heals on any pause longer than one boot; if it bites, the fix is a boot-grace window (treat "redeploy fired < ~120s ago and no heartbeat yet" as active), not a longer cooldown.
- **Both nets used to be no-ops while serve was awake** (open SSE tab = no sleep = no lifespan re-run, and the wake curl did nothing). Closed: `/api/health` now fires the boot re-check hook (internal 5-minute cooldown). Consequence: an external uptime monitor pointed at `/api/health` would wake Neon up to once per 5 minutes - point monitors at the SPA root or a static asset instead.
- **Railway project token blast radius:** the token in serve's env is environment-scoped, not action-scoped - a leak allows mutating any service config in that environment (including the worker's startCommand). Railway offers nothing finer-grained; accepted for a single-operator deployment.
- **GitHub scheduled workflows auto-disable after 60 days without repo activity** - the safety net dies silently if the project goes dormant; re-enable it from the Actions tab when resuming.
