 ...
 * module-level api-mock pattern. AuthGate bypasses entirely for
 * `/share/` paths (see App.tsx), so no getMe/getServerFeatures/getHealth
 * mocking is needed here - only what ShareShell and Compare themselves
 * fetch. useIsMobile is mocked with the vi.hoisted toggle established by
 * MatchShell.test.tsx / RootLayout.test.tsx so the same route can be
 * exercised at both widths; a separate file (not a shared describe in
